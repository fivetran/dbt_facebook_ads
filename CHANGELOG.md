...
[PR #]() includes the following updates: ## Bug Fixes - This package now leverages the new `facebook_ads_extract_url_parameter()` for use in parsing out url parameters. This was added to create special logic for Databricks instances not supported by `dbt_utils.get_url_parameter()`.
